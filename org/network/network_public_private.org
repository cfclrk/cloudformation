...
#+end_src
  ...
  architecture [[https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Scenario2.html][here]]. This is the go-to VPC structure, and is usually considered
  the starting place for any AWS network design. For example, when using the AWS
  CDK, this is the infrastrucure that is created when you use the [[https://docs.aws.amazon.com/cdk/api/latest/docs/@aws-cdk_aws-ec2.Vpc.html][VPC construct]]
...
** Private Subnets
   ...
   The *only* difference between a "public" Subnet and "private" Subnet is that
   public Subntes have a route to an Internet Gateway. There is no quality of a
   Subnet /itself/ that makes it public or private; it's only a matter of what
   routes are in the Route Table that the Subnet is associated with.
